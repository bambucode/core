module Shoppe
  class TaxRatesController < Shoppe::ApplicationController
    ...
    def create
      ...
      if @tax_rate.save
        redirect_to :tax_rates, :flash => {:notice => "Impuesto creado satisfactoriamente"}
      else
        render :action => "form"
      end
    end
    ...
    def update
      if @tax_rate.update(safe_params)
        redirect_to [:edit, @tax_rate], :flash => {:notice => "Impuesto modificado satisfactoriamente"}
      else
        render :action => "form"
      end
    end
    ...
    def destroy
      @tax_rate.destroy
      redirect_to :tax_rates, :flash => {:notice => "Impuesto eliminado satisfactoriamente"}
    end
